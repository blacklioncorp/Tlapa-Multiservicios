...
export const mockPayments: Payment[] = [
  ...
  { id: 'PAY1', servicioId: 'S1A', monto: 150, fechaPago: format(subMonths(today, 0), 'yyyy-MM-dd'), periodoCubierto: format(subMonths(today, 1), 'yyyy-MM'), metodo: 'cash' },
  // Service S1P (Predial, P1) - GREEN
  { id: 'PAY2', servicioId: 'S1P', monto: 500, fechaPago: format(subMonths(today, 1), 'yyyy-MM-dd'), periodoCubierto: format(subMonths(today, 1), 'yyyy-MM'), metodo: 'card' },
  // Service S2A (Agua, P2) - ORANGE
  { id: 'PAY3', servicioId: 'S2A', monto: 180, fechaPago: format(subMonths(today, 2), 'yyyy-MM-dd'), periodoCubierto: format(subMonths(today, 2), 'yyyy-MM'), metodo: 'cash' },
  // Service S2C (Catastro, P2) - GREEN
  { id: 'PAY4', servicioId: 'S2C', monto: 300, fechaPago: format(subMonths(today, 1), 'yyyy-MM-dd'), periodoCubierto: format(subMonths(today, 1), 'yyyy-MM'), metodo: 'transfer' },
  // Service S3A (Agua, P3) - RED
  { id: 'PAY5', servicioId: 'S3A', monto: 160, fechaPago: format(subMonths(today, 4), 'yyyy-MM-dd'), periodoCubierto: format(subMonths(today, 4), 'yyyy-MM'), metodo: 'card' },
